...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            # Get ONLY the target for the last frame
            targets_last_frame = targets[:, -1, :] # Shape: (B, 6)

            loss_pose = criterion(outputs, targets_last_frame)
        ...
        if avg_val_loss < best_val_loss:
            ...
            save_path = os.path.join(checkpoint_dir, "best_model_angle.pt") 
            torch.save({
                'epoch': epoch,
