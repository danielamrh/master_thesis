    ...
    if isinstance(value, torch.Tensor):
        ...
        if len(value) > 0:
            if isinstance(value[0], torch.Tensor):
                print(f"  first element shape: {tuple(value[0].shape)}, dtype={value[0].dtype}")
                print(f"  sample values (first element): {value[0].flatten()[:10]}\n")
            else:
...
# -----------------------------
DOWNSAMPLE_RATE = 16  # render every 20th frame to save memory
